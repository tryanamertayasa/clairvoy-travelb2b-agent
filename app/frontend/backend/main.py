...
Clairvoy B2B Travel Consolidator agent without modifying any core agent files.
...
# Increase timeout for multi-agent B2B travel workflow
adk_agent = ADKAgent(
    adk_agent=root_agent,
    app_name="clairvoy_b2b_travel",
    user_id="demo_user",
...
# Create FastAPI app
app = FastAPI(
    title="Clairvoy B2B Travel Consolidator API",
    description="AG-UI compatible API for the Clairvoy B2B Travel Consolidator agent",
    version="1.0.0",
)
...
async def health_check():
    ...
    return {"status": "healthy", "agent": "ClairvoyB2BTravelConsolidator"}
